...
本文件适用于本仓库及其所有子目录。更深层目录可以增加自己的 `AGENTS.md`，但只能细化规则，不得放宽本文件中的架构、安全和验证要求。

本仓库是 ROS 2 Humble 机器人实时控制域单仓，包含平台共享的机器人模型与 ROS 接口、rt-control 域实现、上游窄补丁、容器部署和目标工控机配置。任何 AI 或人工变更都必须满足：
...
1. 阅读 `README.md`，确认构建、容器启动和仓库布局约束；
2. 阅读 `PROGRESS.md` 中目标任务及相关历史验证记录；
3. 在 `BLOCKED-questions.md` 搜索相关任务号、需求号、设备、接口或配置；
4. 阅读目标包的 `package.xml`、`CMakeLists.txt`、源代码、配置和直接消费者；
5. 执行 `git status --short --branch`，区分用户已有改动与本任务改动；
6. 写明本次变更涉及的任务号/需求号、包、接口、运行阶段和风险等级。

`BLOCKED-questions.md` 中的已裁决项属于权威约束。新需求与既有裁决冲突时必须停止相关实现，记录冲突并请求裁决，不得静默覆盖。缺少电机、驱动器、总线、CPU、标定或安全链事实时，使用 `TBD` 并只阻塞受影响范围，禁止填入“合理默认值”。
...
- `docs`：设计依据、调试记录、部署说明和实机验收证据。

不得在本仓库加入 perception、motion、autonomy 或 gateway 的业务实现，除非架构范围被明确扩展。本仓库中的 rt-control 只负责硬件适配、轨迹/速度/IO 执行、生命周期、故障处理和状态反馈；不得承担任务规划、行为树、导航决策、感知决策、抓取策略或数据库职责。
...
3. **处理阻塞项**：事实缺失或裁决冲突时更新 `BLOCKED-questions.md`，不得自行猜测；
...
8. **记录证据**：更新 `PROGRESS.md` 及相关文档，写清命令、结果、未执行项和原因；
...
3. 更新 `PROGRESS.md` 和必要文档，确保结果可追溯；
...
- 新需求与 `BLOCKED-questions.md`、冻结配置或已验收行为冲突；
