...
def mesh_points_in_footprint(robot, link_name, mesh_name, positions=None):
    ...
    path = PACKAGE_ROOT / mesh.attrib["filename"].removeprefix("package://robot_description/")
    ...
    assert link_name == "base_footprint"
...
def test_legacy_chassis_is_replaced_without_mesh_or_mass_duplication(variant):
    ...
    for name in names:
        ...
        for mesh in link.findall(".//mesh"):
            ...
            assert mesh.attrib["filename"].startswith(f"package://robot_description/{expected}")
