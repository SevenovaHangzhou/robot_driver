...
def render_urdf(end_effector="gripper"):
    result = subprocess.run(
        [
            "xacro", str(PACKAGE_ROOT / "urdf" / "robot.urdf.xacro"),
...
def test_v3_side_zero_and_group_semantics(end_effector):
    ...
    srdf = ET.parse(PACKAGE_ROOT / "srdf" / "robot.srdf").getroot()
    ...
    assert groups["left_arm"].find("chain").attrib == {
        "base_link": "arm_carriage",
        "tip_link": "left_tool0",
    }
    assert groups["right_arm"].find("chain").attrib == {
        "base_link": "arm_carriage",
        "tip_link": "right_tool0",
    }
    assert [joint.attrib["name"] for joint in groups["dual_arm_with_updown"].findall("joint")] == [
        "updown"
    ]
    assert [joint.attrib["name"] for joint in groups["whole"].findall("joint")] == [
        "updown",
        "head_joint",
        "head_pitch_joint",
    ]
    ...
    for pose_name in ("home", "second_home", "unloading", "second_unloading"):
        ...
        arm = {
            ...
            if name.startswith(("left_", "right_")) and "jaw" not in name
        }
        assert states[("dual_arm", pose_name)] == pytest.approx(arm, abs=1e-12)
        assert states[("dual_arm_with_updown", pose_name)] == pytest.approx(
            {"updown": expected["updown"], **arm}, abs=1e-12
        )
        assert states[("whole", pose_name)] == pytest.approx({
            "updown": expected["updown"],
            "head_joint": 0.0,
            "head_pitch_joint": 0.0,
            **arm,
        }, abs=1e-12)
        for index in range(1, 8):
            assert expected[f"right_joint{index}"] == pytest.approx(
                -expected[f"left_joint{index}"], abs=1e-12
            )
    ...
    for link, mesh_name in ((fixed_body, "ee_fixed_body"), (moving_jaw, "moving_jaw")):
        for geometry in ("visual", "collision"):
            ...
            assert link.find(f"{geometry}/geometry/mesh").attrib["filename"] == (
                f"package://robot_description/meshes/robot_v3/{mesh_name}_{geometry}.stl"
            )
...
def test_suction_replaces_gripper_with_source_mass_and_collision_offset(side):
    ...
    for geometry in ("visual", "collision"):
        assert len(link.findall(geometry)) == 1
        assert link.find(f"{geometry}/geometry/mesh").attrib["filename"] == (
            f"package://robot_description/meshes/robot_v3/suction_{geometry}.stl"
        )
    assert link.find("visual/origin").attrib == {"xyz": "0 0 0", "rpy": "0 0 0"}
...
def test_end_effector_switch_preserves_common_robot():
    def common_elements(variant):
        ...
        return {
            ...
            if "moving_jaw" not in element.attrib["name"]
...
def test_variant_entrypoints_and_branch_defaults():
    for variant in ("gripper", "suction"):
        result = subprocess.run(
            ["xacro", str(PACKAGE_ROOT / "urdf" / f"robot_dual_{variant}.urdf.xacro")],
    ...
    published = (PACKAGE_ROOT / "urdf" / "robot.urdf").read_text()
    assert ET.canonicalize(published, strip_text=True) == ET.canonicalize(rendered, strip_text=True)


def test_unknown_end_effector_is_rejected():
    result = subprocess.run(
        ["xacro", str(PACKAGE_ROOT / "urdf" / "robot.urdf.xacro"), "end_effector:=invalid"],
