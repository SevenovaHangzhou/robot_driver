...
TEST(OdometryCovarianceTest, PlacesHealthyBaselinesOnSixBySixDiagonal)
{
  ...
  const auto covariance = make_odometry_covariances(parameters, false, 4U, false);
...
TEST(OdometryCovarianceTest, MultipliesFallbackAndMissingModuleInflation)
{
  ...
  parameters.slip_covariance_scale = 5.0;

  const auto covariance = make_odometry_covariances(parameters, true, 3U, true);

  for (std::size_t index{0U}; index < 6U; ++index) {
    EXPECT_DOUBLE_EQ(covariance.pose[index * 7U], 200.0);
    EXPECT_DOUBLE_EQ(covariance.twist[index * 7U], 400.0);
  }
}
...
TEST(OdometryCovarianceTest, RejectsInvalidPublicInputs)
{
  ...
  EXPECT_THROW(
    static_cast<void>(make_odometry_covariances(parameters, false, 4U, false)),
    std::invalid_argument);
  parameters = OdometryParameters{};
  EXPECT_THROW(
    static_cast<void>(make_odometry_covariances(
        parameters, false, kSwerveModuleCount + 1U, false)),
    std::invalid_argument);
}
