...
TEST(DamiaoProtocolTest, EncodesTrapezoidalProfileWritesAndExplicitFlashSave)
{
  const auto acceleration = make_register_write_float(1U, Register::acceleration, 0.025F);
  EXPECT_EQ(acceleration.id, 0x7FFU);
  EXPECT_EQ(acceleration.data, (std::array<std::uint8_t, 8U>{
    0x01U, 0x00U, 0x55U, 0x04U, 0xCDU, 0xCCU, 0xCCU, 0x3CU}));

  const auto deceleration = make_register_write_float(2U, Register::deceleration, -0.05F);
  EXPECT_EQ(deceleration.data, (std::array<std::uint8_t, 8U>{
    0x02U, 0x00U, 0x55U, 0x05U, 0xCDU, 0xCCU, 0x4CU, 0xBDU}));

  const auto maximum_speed = make_register_write_float(2U, Register::maximum_speed, 20.0F);
  EXPECT_EQ(maximum_speed.data, (std::array<std::uint8_t, 8U>{
    0x02U, 0x00U, 0x55U, 0x06U, 0x00U, 0x00U, 0xA0U, 0x41U}));

  const auto save = make_parameter_save(2U);
  EXPECT_EQ(save.id, 0x7FFU);
  EXPECT_EQ(save.data, (std::array<std::uint8_t, 8U>{
    0x02U, 0x00U, 0xAAU, 0x01U, 0U, 0U, 0U, 0U}));
  EXPECT_THROW(
    static_cast<void>(make_register_write_float(1U, Register::acceleration, NAN)),
    std::invalid_argument);
}

TEST(DamiaoProtocolTest, ValidatesStoredTrapezoidalProfileReadback)
{
  const TrapezoidalProfile expected{0.025, -0.05, 20.0};
  EXPECT_TRUE(expected.valid());
  EXPECT_NO_THROW(validate_trapezoidal_profile(expected, expected));
  EXPECT_THROW(
    validate_trapezoidal_profile(TrapezoidalProfile{0.03, -0.05, 20.0}, expected),
    std::invalid_argument);
  EXPECT_FALSE((TrapezoidalProfile{-0.025, -0.05, 20.0}.valid()));
  EXPECT_FALSE((TrapezoidalProfile{0.025, 0.05, 20.0}.valid()));
  EXPECT_FALSE((TrapezoidalProfile{0.025, -0.05, 0.0}.valid()));
}
