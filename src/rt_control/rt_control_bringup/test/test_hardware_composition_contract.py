...
ECAT_SENSOR_INTERFACES = {
    "right_force_sensor": X503_STATE_INTERFACES,
    "left_force_sensor": X503_STATE_INTERFACES,
    ...
    **{
        ...
        for ring_position in (*range(1, 13), 14, 15, 16, 17)
    },
}
ECAT_AXIS_BINDINGS = (
    ...
    ("turn", 16, "turn"),
    ("updown", 17, "xmc_updown_sw511"),
)
ECAT_PASSIVE_SENSOR_BINDINGS = (
...
def test_real_ethercat_joint_and_sensor_interface_contract(
    ...
    assert set(sensors) == set(ECAT_SENSOR_INTERFACES)
    assert {
...
def test_real_ethercat_driver_configuration_contract(
    ...
    passive_sensor_names = {
        sensor_name
        for sensor_name, _ring_position, _profile in ECAT_PASSIVE_SENSOR_BINDINGS
    }
    actual_sensor_bindings = []
    for sensor_name, sensor in _sensors(system).items():
        if sensor_name not in passive_sensor_names:
            continue
        assert sensor.findall("ec_module") == []
        parameters = _parameters(sensor)
        assert parameters["ec_module.name"] == f"{sensor_name}_device"
        assert parameters["ec_module.plugin"] == (
            "ethercat_generic_plugins/GenericEcSlave"
        )
        assert parameters["ec_module.alias"] == "0"
        config_path = Path(parameters["ec_module.slave_config"])
        assert config_path.parent.name == "slaves"
        actual_sensor_bindings.append(
            (sensor_name, int(parameters["ec_module.position"]), config_path.stem)
        )
    assert tuple(actual_sensor_bindings) == ECAT_PASSIVE_SENSOR_BINDINGS
...
def test_mock_ethercat_joint_and_sensor_interface_contract(
    ...
    for sensor_name, _position, _profile in ECAT_PASSIVE_SENSOR_BINDINGS:
        sensor = sensors[sensor_name]
        assert all(
            _parameters(interface) == {"initial_value": "0.0"}
...
def test_diagnostics_composition_matches_expanded_hardware_graph(
    ...
    ecat_sensor_topology = [
        (sensor_name, int(_parameters(sensor)["ec_module.position"]))
        for sensor_name, sensor in _sensors(real_systems["ecat_arms"]).items()
        if sensor_name in passive_sensor_names
    ]
    assert [
