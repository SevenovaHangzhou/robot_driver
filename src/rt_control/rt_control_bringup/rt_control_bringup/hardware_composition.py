...
def _parse_ethercat_variant(path: str | Path) -> EthercatComposition:
    ...
    root = _descriptor_header(
        ...
        "ecat_arms",
        frozenset(
            {"schema_version", "variant", "system", "extra_responders", "axes"}
        ),
    )
    ...
    return EthercatComposition(expected_responders, tuple(axes))
...
def load_hardware_variants(
    ...
    overlap = {axis.joint_name for axis in ethercat.axes} & {
        node.joint_name for node in canopen.nodes
    }
    if overlap:
        names = ", ".join(sorted(overlap))
        raise HardwareCompositionError(
            f"joint_name values must be unique across buses: {names}"
        )
