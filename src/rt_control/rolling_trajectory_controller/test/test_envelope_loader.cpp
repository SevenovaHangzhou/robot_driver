...
TEST(EnvelopeLoader, LoadsExactFourteenAxisProvisionalArtifactAndHashesRawBytes)
{
  ...
  EXPECT_EQ(
    ...
    "1673393117a50ed04b7b9fa99dd01dbeb171cc691e9733a65e49fb13b418baab");

  EXPECT_DOUBLE_EQ(envelope.axes[0].position_lower, -3.14159265);
  EXPECT_DOUBLE_EQ(envelope.axes[3].position_upper, 2.53072742);
  EXPECT_DOUBLE_EQ(envelope.axes[12].velocity_positive, 0.1);
  EXPECT_DOUBLE_EQ(envelope.axes[13].position_lower, -3.14159265);
  EXPECT_DOUBLE_EQ(envelope.axes[13].position_upper, 3.14159265);
  EXPECT_DOUBLE_EQ(envelope.axes[13].velocity_positive, 0.1);
...
TEST(EnvelopeLoader, RejectsWrongSourceAxisOrderUnknownFieldsAndInvalidNumbers)
{
  ...
  const std::vector<std::pair<std::string, std::string>> mutations = {
    ...
    {"    position_lower: -3.14159265",
      "    unexpected_limit: 1.0\n    position_lower: -3.14159265"},
    {"velocity_positive: 0.1", "velocity_positive: 0.0"},
    {"acceleration_positive: 0.2", "acceleration_positive: .nan"},
    {"position_margin_lower: 0.01", "position_margin_lower: -0.1"}};
