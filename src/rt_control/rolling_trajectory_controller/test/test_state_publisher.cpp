...
constexpr std::array<std::uint8_t, 32> kAxisSetHash = {
  0xf4U, 0xc8U, 0xffU, 0x8aU, 0x32U, 0x18U, 0x3dU, 0x17U,
  0x33U, 0x03U, 0x24U, 0x94U, 0x60U, 0x0cU, 0x9fU, 0x5dU,
  0x1eU, 0x7eU, 0x62U, 0x56U, 0x73U, 0xa4U, 0xadU, 0x51U,
  0x55U, 0x6bU, 0x3aU, 0xc6U, 0xc3U, 0x5aU, 0xdeU, 0xe4U};
...
TEST(StatePublisherProvisionalTest, PublicStateExposesProvisionalSourceAndFileHash)
{
  ...
  EXPECT_EQ(
    ...
    (std::array<std::uint8_t, 32>{
        0x16U, 0x73U, 0x39U, 0x31U, 0x17U, 0xa5U, 0x0eU, 0xd0U,
        0x4bU, 0x7bU, 0x9fU, 0xa9U, 0x9dU, 0xd0U, 0x1dU, 0xbeU,
        0xb1U, 0x71U, 0xccU, 0x69U, 0x1eU, 0x97U, 0x33U, 0xa6U,
        0x5eU, 0x49U, 0xfbU, 0x13U, 0xb4U, 0x18U, 0xbaU, 0xabU}));
  EXPECT_EQ(
...
TEST_F(StatePublisherTest, ConfiguredWallTimerPublishesAfterControllerActivation)
{
  ASSERT_EQ(update(), controller_interface::return_type::OK);
  auto observer = std::make_shared<rclcpp::Node>("rolling_state_timer_observer");
  std::atomic_uint32_t received_count{0U};
  auto subscription = observer->create_subscription<State>(
    "/rt/rolling_joint_control/state", rclcpp::QoS(5).reliable(),
    [&received_count](State::SharedPtr) {
      received_count.fetch_add(1U, std::memory_order_release);
    });
  ...
  while (
    received_count.load(std::memory_order_acquire) == 0U &&
    std::chrono::steady_clock::now() < deadline)
  {
    executor.spin_some();
    std::this_thread::sleep_for(1ms);
  }
  EXPECT_GT(received_count.load(std::memory_order_acquire), 0U);
