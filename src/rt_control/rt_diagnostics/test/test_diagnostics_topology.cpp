...
TEST(DiagnosticsTopologyTest, PreservesConfiguredOrderForNoncontiguousTopology)
{
  const auto topology = DiagnosticsTopology::create(
    {"axis_c", "axis_a", "axis_b"}, {7, 1, 11},
    {"force_right", "force_left"}, {8, 9}, 13, {42, 7, 11});
  ...
  ASSERT_EQ(topology.ethercat_sensors().size(), 2U);
  EXPECT_EQ(topology.ethercat_sensors()[0].sensor_name, "force_right");
  EXPECT_EQ(topology.ethercat_sensors()[0].ring_position, 8U);
  EXPECT_EQ(topology.ethercat_sensors()[1].sensor_name, "force_left");
  EXPECT_EQ(topology.ethercat_sensors()[1].ring_position, 9U);
...
struct InvalidTopology
{
  ...
  std::vector<std::int64_t> sensor_ring_positions;
...
TEST_P(InvalidDiagnosticsTopologyTest, RejectsUnsafeOrAmbiguousTopology)
{
  ...
  EXPECT_THROW(
    DiagnosticsTopology::create(
      ...
      topology.sensor_names, topology.sensor_ring_positions,
...
INSTANTIATE_TEST_SUITE_P(
  ...
  ::testing::Values(
    ...
    InvalidTopology{{"a"}, {1}, {}, {}, 1, {2}},
    InvalidTopology{{"a"}, {0}, {"force"}, {}, 2, {2}},
    InvalidTopology{{"a"}, {0}, {"force", "force"}, {1, 2}, 3, {2}},
    InvalidTopology{{"a"}, {0}, {"a"}, {1}, 2, {2}},
    InvalidTopology{{"a"}, {0}, {"force"}, {0}, 2, {2}},
