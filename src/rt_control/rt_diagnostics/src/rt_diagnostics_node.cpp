...
constexpr std::array<std::uint8_t, 2> kCanopenNodeIds = {2U, 3U};
constexpr std::size_t kExpectedEthercatResponders = 16U;
...
private:
  ...
  void onDiagnostics(const diagnostic_msgs::msg::DiagnosticArray::SharedPtr message)
  {
    ...
    for (const auto & status : message->status) {
      ...
      const auto node = std::find_if(
        kCanopenNodeIds.begin(), kCanopenNodeIds.end(),
        [&status](const std::uint8_t node_id) {
          return status.hardware_id == std::to_string(node_id);
        });
      if (node == kCanopenNodeIds.end()) {
        continue;
      }
      ...
      const auto index = static_cast<std::size_t>(
        std::distance(kCanopenNodeIds.begin(), node));
  ...
  void publishSnapshot()
  {
    ...
    std::array<CanopenSnapshot, kCanopenNodeIds.size()> canopen;
    SteadyClock::time_point received;
    ...
    if (topic_stale || process_stale || !master_complete) {
      ...
      static_cast<int>(slaves_responding) !=
      static_cast<int>(kExpectedEthercatResponders) ||
    ...
    for (std::size_t node = 0; node < canopen.size(); ++node) {
      ...
      normalized.name = "/robot/rt_control/canopen/node_" + std::to_string(node_id);
      ...
      normalized.values.push_back(
        makeKeyValue("source_hardware_id", std::to_string(node_id)));
  ...
  std::array<CanopenSnapshot, kCanopenNodeIds.size()> canopen_{};
