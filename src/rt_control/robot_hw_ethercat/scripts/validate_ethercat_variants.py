...
def _required_interfaces(
    raw_interfaces: Any, context: str, *, state: bool, allow_empty: bool = False
...
def validate_variant(variant_path: Path, profile_dir: Path) -> None:
    ...
    validated_profiles: dict[tuple[Path, str, int | None], None] = {}
    ...
    for index, raw_axis in enumerate(axes):
        ...
        if not family_contract.interface_contract.command:
            raise ValidationError(
                f"{context}: family {family!r} cannot register a motion axis"
            )
    ...
    sensors = _sequence(descriptor["sensors"], f"{variant_path}: sensors")
    ...
    for index, raw_sensor in enumerate(sensors):
        ...
        if sensor_name in seen_sensors:
            raise ValidationError(
                f"{variant_path}: duplicate sensor_name {sensor_name!r}"
            )
        if sensor_name in seen_joints:
            raise ValidationError(
                f"{variant_path}: resource names must be unique across axes and sensors"
            )
        seen_sensors.add(sensor_name)

        position = _ring_position(sensor["ring_position"], f"{context}.ring_position")
        if position in seen_positions:
            raise ValidationError(f"{variant_path}: duplicate ring_position {position}")
        seen_positions.add(position)

        family = _identifier(sensor["family"], f"{context}.family")
        if family not in families:
            raise ValidationError(f"{context}: unsupported family {family!r}")
        family_contract = families[family]
        if family_contract.interface_contract.command:
            raise ValidationError(
                f"{context}: passive sensor family {family!r} must not expose command interfaces"
            )
        if family_contract.certified_modes:
            raise ValidationError(
                f"{context}: passive sensor family {family!r} must not declare certified modes"
            )

        profile_name = sensor["profile"]
        if not isinstance(profile_name, str) or PROFILE_PATTERN.fullmatch(
            profile_name
        ) is None:
            raise ValidationError(f"{context}: invalid profile name {profile_name!r}")
        ...
        profile_key = (resolved_profile_path, family, None)
        if profile_key not in validated_profiles:
            _validate_required_interface_bindings(
                profile_path, profile, family_contract.interface_contract
            )
