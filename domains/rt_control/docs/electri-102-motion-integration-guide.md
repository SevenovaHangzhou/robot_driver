# ELECTRI-102 给运控的视觉伺服对接说明

> 一句话结论：不要向驱动持续发送 `TwistStamped`。运控需要每秒发送 30 次“未来约
> 0.5 秒的短轨迹”，每条轨迹包含完整 14 轴；驱动负责检查轨迹，并以 250 Hz 插值执行。
>
> 当前只完成公共接口和隔离 mock 验证，尚未授权真实机器人运动。

## 0. 先看这一页

如果你第一次接这个接口，先看本节就够了。后面的内容是字段、错误码和调试细节，写代码或
排查问题时再查。

### 0.1 到底要做什么

视觉算法通常输出末端速度或关节速度，但驱动不直接接这个速度。运控要把速度整理成一小段
关节轨迹，再交给驱动。

可以把它理解成不断改写“还没执行的后半段路线”：

1. 运控手里始终留着未来约 500 ms 的轨迹；
2. 每隔约 33 ms，也就是 30 Hz，重新算一次后面的路线；
3. 新路线从驱动允许替换的时间开始，覆盖尚未执行的旧路线；
4. 驱动在两个轨迹点之间平滑插值，并以 250 Hz 给关节下发位置。

轨迹点本身每 100 ms 一个，所以一包通常是 6 个点：
...
“100 ms 一个点”和“每秒发 30 包”不是一回事：前者是轨迹点之间的时间，后者是运控修改
未来轨迹的频率。
...
| 功能 | 用人话解释 |
| --- | --- |
| 会话管理 | 在 FJT 和视觉伺服之间切模式，负责 open、close 和异常退出。 |
| 轨迹缓存 | 保存“驱动最后确认接受”的那条轨迹，后续更新都以它为准。 |
| 轨迹整形 | 把视觉算法输出限速、限加速度，再积分成位置和速度点。 |
| 30 Hz 发布 | 每次发送约 500 ms 的完整 14 轴短轨迹。 |
| 状态与错误处理 | 根据驱动 state 判断是否接受；被拒绝时重新计算，不盲目重发。 |

其中最重要的是“驱动最后确认接受的轨迹”。消息 publish 成功只说明 DDS 收到了数据，不说明
驱动接受了它。只有 state 中的 `last_accepted_sequence` 追上本批 sequence，这批轨迹才能
成为下一次计算的基准。
...
1. 取消运控自己发出的 FJT goal，并等到 Action 最终结束；
2. 调 `set_mode`，从 FJT 切到 rolling；
3. 调 `open`，读取驱动实际返回的 session ID、时间范围和 hold 位置；
4. 不要等下一帧图像，立即发送一包 6 点的静止轨迹，让会话进入 RUNNING；
5. 正常视觉伺服期间，每秒发 30 包短轨迹；
6. 视觉短时丢失时继续发包，平滑减速到保持，不要直接停发；
7. 退出时先请求减速停止，等状态变成 HOLDING，再 finalize；
8. session 清空后，调用 `set_mode` 切回 FJT。

任何一步失败，都先读公共 state 确认现在到底在哪个模式、哪个 session；不要猜，也不要自动
重放上一包。

### 0.4 每一包轨迹怎么生成

驱动会在 state 中给出最早可替换时间 `replaceable_from_ns`，把它记为 R。这个时间直接用，
不要拿系统时间或 ROS 时间自己换算。

生成新轨迹时：

1. 从“最后确认接受的旧轨迹”中采样 R 时刻的 14 轴位置和速度；
2. 新轨迹第 0 点必须原样使用这组位置和速度，保证新旧路线接得上；
3. 只修改当前正在视觉伺服的那条手臂；
4. 另一条手臂、Turn 和 Updown，也必须按未来每个时刻从旧轨迹中取值并填进去；
5. 对伺服臂做限速、限加速度和位置积分，生成后续 5 个点；
6. 发布后等 state 确认接受，再更新本地的“已接受轨迹”。

本期一次只伺服一条手臂，但消息永远是完整 14 轴。固定顺序是：

`right_joint1..6, left_joint1..6, turn, updown`。

### 0.5 最容易犯的六个错误

- **拿 `/joint_states` 填非伺服轴**：不对。那是当前实测值，不是未来计划值，容易把仍在
  执行的轴突然拉回去。
- **Open 后等下一帧图像再发第一包**：不对。Prime 超时只有 100 ms，应立即用 hold 值发包。
- **publish 成功就认为驱动接受**：不对。必须等 `last_accepted_sequence`。
- **上一包被拒绝后复用 sequence**：不对。每次尝试 sequence 都递增，然后重新计算。
- **视觉丢帧就停发**：不对。运控应继续发平滑减速或保持轨迹；200 ms timeout 只用于真正
  的进程或通信失联。
- **把驱动的容差当平滑目标**：不对。驱动只负责拒绝明显不连续的轨迹，真正的平滑仍由运控
  保证。
...
| 项目 | 当前值 | 运控怎么用 |
| --- | ---: | --- |
| 轨迹点间隔 | 100 ms | 每包按这个间隔生成点。 |
| 发包频率 | 30 Hz | 大约每 33 ms 重算并发布一包。 |
| 计划未来长度 | 约 500 ms | 当前通常生成 6 个点。 |
| 旋转关节速度上限 | 15 deg/s | 只用于 rolling 视觉伺服。 |
| 旋转关节加速度上限 | 0.75 rad/s² | 运控生成轨迹时主动限制。 |
| 驱动更新超时 | 200 ms | 只作失联兜底，不是视觉丢帧策略。 |

Open response 会返回本次真正生效的 capacity、horizon、timeout 和 replace lead。代码应使用
response，不能把上表当成永远不变的硬编码。

### 0.7 双方责任怎么分

运控负责：视觉控制律、IK、碰撞约束、限速限加速度、轨迹连续、视觉丢失时如何减速，以及
完整 14 轴 future 的生成。

驱动负责：检查协议、时间、连续性和安全包络；保存可执行短轨迹；250 Hz 插值；超时或低水位
时按已验证路径停车。

驱动不会替运控把一条突然变化的速度命令“修平”，也不会替运控决定视觉丢失后的业务动作。
