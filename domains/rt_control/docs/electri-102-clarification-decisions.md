...
**问题**：对缺少现场授权／权威输入、或已由 D-06 等裁决另行立项的能力，本期是继续扩大
ELECTRI-102 范围、只在口头上搁置，还是形成明确的延期边界？

- **【备选 A】** 全部纳入本期并以“视觉伺服完整完成”为终点。范围最完整，但手眼标定、
  目标机测量和硬件运动需要当前并不存在的权威数据与单独授权，也会推翻已冻结的不在本期项。
- **【备选 B】** 代码做到哪里算哪里，不在交付物中列延期项。文档较短，但后续人员无法区分
  软件缺陷、外部阻塞和明确延期，容易把 mock 结论误外推为生产验收。
- **【推荐／最终采用 C】** 把 rolling software／mock 作为本期准确完成边界，逐项登记延期能力、
  关闭条件与所需授权；这些能力不得借当前分支隐式进入范围，也不得反向阻塞已完成的软件门。

因此，以下内容不是未完成的软件选择，而是明确不在本期或需要外部证据：
