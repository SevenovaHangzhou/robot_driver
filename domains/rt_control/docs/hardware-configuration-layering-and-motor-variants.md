...
- 变体注册表驱动本总线的 real/mock Xacro，Xacro 通过 ros2_control
  `HardwareInfo` 把运动轴或被动 sensor、地址、profile 和 mode 交给通用 hardware
  plugin；
...
`ethercat_generic_plugins/EcCiA402Drive`，X503 sensor 使用
`ethercat_generic_plugins/GenericEcSlave`。descriptor 中的 `profile` 是经验证的设备
...
- 被动 sensor 必须在 descriptor 的独立 `sensors` 列表注册，不能伪装为 motion axis
  或匿名 responder；
...
flowchart TD
    ...
    ECMAP["robot_hw_ethercat/variants/name.yaml<br/>axis/sensor / family / ring / profile / CSP8"]
...
| `robot_hw_ethercat/variants/*.yaml` | system、extra responders、运动轴 joint/family/ring/profile/mode 与被动 sensor name/family/ring/profile 注册；同一描述符派生 real/mock 资源与诊断 topology | profile 内部 PDO/SDO、JTC joints、使能批次 |
...
3. bringup 从 EtherCAT `axes + sensors + extra_responders` 计算 responder 数量，并从
   两份 descriptor 提取 joint/sensor/ring/node 列表，只作为 `rt_diagnostics` 的只读
   参数。
4. 总装 Xacro 分别把两个 selected variant 名传给对应 hardware package。每个包自己的
   公开宏加载自己的 descriptor，生成 real/mock ros2_control system。
5. 真实分支把 descriptor 值编码为 `HardwareInfo`：EtherCAT 每个 axis/sensor 提供
   `ec_module.*`，CANopen 每 node 提供 `node_id` 和 `operation_mode`。通用 plugin
   只消费这些类型化参数，不识别 `alfa_v1`。
...
| 要修改的事实 | Owner 文件 |
| --- | --- |
| EtherCAT 轴、被动 sensor、环位、family、profile 与 mode 注册 | `src/rt_control/robot_hw_ethercat/variants/alfa_v1.yaml` |
...
增加或删除 X503 这类 state-only sensor 时，修改 descriptor 的 `sensors` 列表及对应
family/profile，并验证 state interface、完整 ring、diagnostics 与 real/mock 对齐；不得
把 sensor 加入 Robot Model 运动 joint、JTC、enable_manager 或 command interface。
若 sensor 的工程单位、通道语义或解耦矩阵尚未确认，只能继续发布已批准的 raw state。
...
| 增加/删除 state-only EtherCAT sensor | 不可以 | sensor profile/family + descriptor + diagnostics/Mock；不得进入运动策略 |
...
### 8.1 未合并分支的 2026-08-19 历史 T1 基线
...
### 8.2 2026-09-03 双 X503 main 移植 T1 基线

当前移植以 `main@b80f6c1` 为底，完整保留 PR #16 的 18 位拓扑，并由 descriptor
独立登记 14 个 motion axes、两台 X503 sensors 和两个 Hub responders。完整候选镜像
构建 29 个包；镜像 ID、365 项 in-image 测试、隔离 Mock 与停机证据见
[`release-deploy-20260903-01`](areas/release-deploy/records/2026-09-03-port-hardware-composition-to-main.md)。

该候选镜像来自未提交 worktree，只是 T1 验证证据。必须在提交/CI 后以最终 SHA 重建，
且 PR #16 要求的 18 位不使能实机 OP/WC/X503 raw 检查仍未由本次移植完成。
