...
## BQ-147: 三代机 PP 夹爪标定与实机准入 [OPEN/HIGH-RISK 2026-09-08]

- 用户批准 ELECTRI-118 复用官方位置夹爪 controller，补最大力下发与零差 PP 适配。
- 软件按标准 GripperCommand 使用 m/N；6072 实际为额定电流千分比。不得把原始电流或
  电机扭矩数值直接当成夹爪开口/夹持力。当前只支持经确认的仿射位置标定和全行程保守限力换算。
- 待确认：真实夹爪型号/固件与身份、PDO assignment、6072 的类型/单位及 PP 下限值生效时序、
  606C/6077 反馈比例、传动/零位/方向/行程、限力及停止阈值、6081/6083/6084/605D 参数。
- 厂家手册第 198 页与旧 ESI 对部分限矩/电流对象的类型/权限描述不一致，不能仅凭通用 CiA402
  定义宣布实机映射已确认。60E0/60E1 的总线力控前提未确认，本实现不启用这两个对象。
- 两份 draft 配置保留 TBD 且 hardware `verified: false`；alfa_v3 runtime gate 不放开。
  Robot Model、实际 physical profile 和 scope-specific controller 生命周期接入另行闭合。
- 2026-09-12 只读核验：16 台均为 mode 8，6502=0x38D；直接 CoE 回读确认当前
  1600/1A00 分别为 607A/60FE/6040 和 6064/60FD/6041，各 10 bytes。
  全部 6072 字典类型为 int16，与 ESI 相符；适配器已修正为正有符号范围，并要求 PP
  TxPDO 最后添加一个 8bit dummy，得到 14-byte 反馈。新 PP 映射尚未写入实机。
- 605D 在全部 16 台上返回 0x06020000（对象不存在），不能使用手册值或删除要求绕过
  Halt/cancel 准入；需要零差确认夹爪固件 550403170/550729090 的停止行为和限力语义。
  位置 3/8/12/16 的 603F=0x730F（手册：多圈保持电池电压低）需现场检查，未执行复位。
  详细回读、修正和厂家问题见 ecat-axes-20260912-05。
- 用户确认左 J3（3）、右 J4（12）仅需单圈，两个夹爪（8/16）需要多圈。官方
  eRob 用户手册 V3.39 §9.4 允许多圈型号按单圈使用，经 6040h 故障复位后使能，
  前提是不跨输出端单圈边界；允许清错不等于忽略 6041 Fault 或位置校验。
- 用户补装电池后只读复查：夹爪 603F 从 730F 变为 7314（电池重新接入），左 J3/右 J4
  仍为 730F。夹爪电池/多圈参考尚未验证；2242 负载编码器复位与普通 6040 清错有别，
  不自动执行或把它加入启动 SDO。策略与待执行操作见 ecat-axes-20260912-06。
- 用户随后批准夹爪 8/16 的各一次 2242 复位。位置 8 已写一次并返回成功；首次回读
  I/O error 后通讯恢复，603F=0、2242=1，但 6041=0208/1208（Fault）、1001=1 持续。
  按异常停止条件暂停后续写入，位置 16 尚未复位。普通 6040 清错未执行，具体下一步
  见 ecat-axes-20260912-07；不把“错误码为零”等同于“驱动已恢复”。
- 用户追加允许一次 6040 普通清错，位置 8 已按 0000->0080->0000 各写一次，三次
  SDO 均成功，但六次回读和后续诊断仍为 Fault/1001=1/603F=0。未达到继续右侧的条件。
  1003 历史左侧含 7377/7314/7315、右侧含 7377/7314，未确认发生时间与因果；不把
  历史码冒充当前错误。需厂家确认该固件的 PREOP 清错和 2242 复归语义，见
  ecat-axes-20260912-08；未重复写入、未重启、未启动 OP 或使能。
- 用户用零差上位机重置负载端编码器后，两夹爪 603F 均为 0，仍 Fault。用户新授权
  两侧普通清错，本轮位置 8/16 各执行一次 6040=0->128->0；置位期间都回读到 128，
  但五次后续回读仍 Fault/1001=1/603F=0。最终控制字均 0，2242 左=1/右=0；
  不能据此认定当前根因。详细记录与上位机普通清错对照方案见 ecat-axes-20260912-09。
- 用户随后重新上电；UTC 08:26 的两次只读检查确认两夹爪 603F/1001 均归零，
  Fault 位清除、2242=0，未再次发清错。全部 16 轴 Fault 位为 0，仍为模式 8，
  仅位置 3/12 保留 730F。当前夹爪故障已解除，原先“仍 Fault”的快照为历史状态；
  未进入 PP，不能归因于 PP。完整记录见 ecat-axes-20260912-10，运行与标定准入仍待闭合。
- 后续用户确认当前机械零位与无额外传动比，明确授权 14 CSP + 2 PP 使能。原位入口
  已真实通过：两个夹爪 mode=1，全部 16 轴 OperationEnabled，WC 48/48，目标保持；
  /rt/disable 和有序停机成功。当前已失能、主站 Idle/Inactive，参见 ecat-axes-20260912-11。
  此入口不触发 PP 新目标，不用 605D/力控制 Action；这些对象的运动语义和标定仍开放。
  J3/J4 的 730F 在 OP 后通过现有 /rt/reset_fault(PDO) 清除成功，未忽略 Fault 状态。
- 离线编译/Action/PDO 测试仅验证软件语义；急停、驱动保护、实际夹持力、停止距离及持物安全
  仍需现场授权后的实机验证。本项不授权使能、写 SDO 或运动。

## BQ-144: 三代机舵轮保留转向 CSP，外置编码器不闭合位置外环 [RESOLVED/DESIGN 2026-09-08]

- User decision：选择保留转向 CSP 的路线。4 个步科转向电机使用 CSP(8)，4 个驱动轮电机
  保持 CSV(9)，与机械臂/Updown 共用现有 IgH master 0。用户确认当前驱动器不支持外置编码器输入。
- User decision：算法包名使用 `swerve_driver`，不保留 `dm_swerve_driver` 前缀；迁移时删除达妙
  `rezero` 服务/流程。四个 CANopen 编码器使用 SYNC 触发同步 TPDO，目标采样周期按 4 ms。
- 外置 CANopen 编码器只测转向输出轴。经齿比、方向、安装偏移换算后，作为实际舵角主数据源，
  用于上电绝对方向/坐标映射校验、舵角优化、余弦补偿、对齐门控、里程计和异常检测。
- 转向电机自身编码器继续参与驱动器内部位置/速度反馈，并用于 CSP 电机位置坐标及交叉校验。
  必须分别保留电机位置和输出轴实测舵角，不能把外置读数直接覆盖电机 position state interface。
- 不增加 CSP 上的慢速积分修正、实时背隙补偿、转向 CSV 位置外环或驱动器内部双反馈闭环。
  不在运动中重写编码器零点；上电校验也不隐含自动覆盖已批准的标定参数。
- User decision：新底盘包删除达妙 MIT `kp/kd` 和驱动轮 `ks/kv/ka` 力矩前馈的配置项、计算及
  命令输出，不将它们作为占位参数保留。转向 CSP 使用步科内部位置/速度环，驱动 CSV 使用其内部
  速度环；步科内部环参数单独按厂商对象整定，不沿用 MIT 参数的名称或数值，也不在每周期重写增益。
- 4 ms 是已确定的 CANopen SYNC 设计周期；四个编码器响应同步型 TPDO，不据此宣称 CAN 报文
  同时到达或与 EtherCAT DC 硬件同相。实际设备支持、采样/传输时延、抖动和反馈年龄门限仍待核验。
- `owner_refs` 按硬件协议登记，不放入底盘算法包名。`swerve_driver` 源码与控制器已于 2026-09-09 迁入；
  头部 `damiao_can` 的既有协议引用不因底盘更名而被删除或重新分配。
- 本决定取代本轮讨论中的慢速修正建议，以及达妙分支 Phase 8 中将外置舵角观测直接描述为
  已闭环消除背隙/弹性的表述。实际输出轴误差可用于门控/告警，但本方案不承诺主动消除该误差。
- 未解决的硬件事实不由本决定补全：真实 PDO/SDO、精确齿比、方向、零位、反馈周期、差异容差与
  停车/降级条件仍须核验和标定；不新增自动故障恢复或掉线后继续运动的授权。实机 runtime gate 保持关闭。
...
- 用户提供 INOVANCE-GR10-EC-6SW-1.4.2.3.xml，ESI 确认型号为 GR10-EC-6SW，修正此前输入的
  GR10-EC-65W。文件已归档在 robot_hw_ethercat/config/esi，原始及归档哈希见同目录 README。
- ESI vendor=0x00100000，两个 Device 的 product code 分别为 0x10F40931、0x10F40932，
  revision 均为 0x00010000。子设备通过主设备内部 EBUS port 2 关联；外部连接器为 IN/X2..X6。
- 文件未声明 PDO/Mailbox/SM；声明 DC(0x0300) 与 Synchron(0) 选项。此处不据此选择运行模式，
  不将分支器计入电机使能清单。若现场仅一台该分支器加 16 电机，预计 18 响应设备，须实际扫描核验。
- 旧机 Hub 的 vendor/product/revision、端口接线、ring position 和 OP/PREOP 例外不能移用到新分支器。
- 双臂仍为 16 个执行器（14 CSP + 2 PP）；整条 EtherCAT 总线的可见从站数应包含实际出现的
  分支器/其他响应设备，须由 ESI 与现场拓扑交叉核验，不能将执行器数直接作为总线从站总数。
- 2026-09-12 用户确认左臂 X2、右臂 X3；只读扫描得到 18 个 PREOP 从站，master Idle/Inactive。
  分支器主/子设备位于绝对位置 0/17，身份三元组与 ESI 一致，存储 alias 分别为 2/7。
  主 port 3(X2) -> 1、port 1(X3) -> 9、port 2(内部) -> 17；两条八电机链分别占 1..8/9..16。
  十六电机均读到 0x5A65726F/0x00029252@0x00000001、Serial=0，不能凭相同身份推断关节/夹爪。
- 用户随后确认两条链均为 J1..J7 -> 夹爪；左臂 1..7 与右臂 9..15 设定 CSP，
  8/16 为 PP 夹爪。owner-local 物理清单保存在 config/machines/alfa_v3_arms_only.draft.yaml，
  这是目标模式分配，不是当前 0x6061 回读；正式 Robot Model joint 名称仍为 TBD。
- 待核实：PDO/SDO、运行状态与 DC/初始化配置，以及设备名称
  1.4.2.0 与 ESI 名称 1.4.2.3 的差异；名称不是身份 revision。扫描事实见
