...
| 维度 | 结论 |
|---|---|
| 整机形态 | 移动复合机器人 = 自主导航底盘 + 机械臂，视觉引导抓取箱子（搬运/上下料） |
| 软件框架 | ROS 2 |
    ...
    subgraph ROBOT["机器人本体"]
        ...
        subgraph B["B 域 · 实时控制域<br/>x86 工控机 Ubuntu 24.04 + PREEMPT_RT"]
            subgraph BRT["实时分区（隔离核，systemd 原生）"]
                IGH["IgH EtherCAT 主站"]
                CM["controller_manager 1kHz<br/>JTC / diff_drive / 安全监控"]
            end
...
| L1 | 实时总线与控制层 | EtherCAT 主站 + CiA 402 + 1kHz 控制环（**唯一重自研区**） | IgH Master、自研驱动库、ros2_control、JTC/diff_drive | 硬实时 1kHz |
...
- **非实时 → 实时**（下行运动指令仅两条）：`cmd_vel` → `diff_drive_controller`；`FollowJointTrajectory` → `joint_trajectory_controller`。上行状态通道（`/joint_states`、odom+TF、`/diagnostics`、驱动器故障事件）单独列明——注意 FollowJointTrajectory 是 action，天然有 feedback/result 回流，"单向"指的是运动指令的方向。
...
4. 底盘刹车、抱闸，行为树确认 `base_parked` 后才允许臂动作（互锁详见 §10.6）；
...
7. 行为树确认 `arm_stowed` 后才允许底盘移动，导航到目标点、放置（对称流程）、上报任务完成；
...
| EtherCAT 链路断裂 | 主站/从站 WDT | 全部从站失联 → 整机受控停机（单链拓扑的已知代价，见 §5.2） |
...
- **EtherCAT 菊花链**（B 域网口 1 引出）：轮驱 ×2/4 → 臂 J1–J6 → 夹爪/数字 IO → 安全状态回读从站，全链 DC 分布式时钟同步。**已知代价**：单链任一点断裂（尤其穿过臂关节的运动电缆，疲劳断裂高发）→ 全部从站失联 → 整机受控停机。第一版接受此行为，量产前评估分链（底盘/臂两条链，需多口 EtherCAT 卡）或线缆冗余（环形拓扑，注意 IgH 对冗余的支持有限）。
...
- **急停链路**：双通道急停按钮（车身多点 + 手持盒，双 NC 触点）+ 安全雷达 OSSD → 可编程安全控制器（SICK Flexi Soft / Pilz PNOZmulti 2，比纯继电器贵但区域逻辑可配置）→ 各伺服驱动器 **STO/SS1** 端子 + 主接触器。全链独立布线，不走以太网。选驱动器时"带证书的 STO"是硬性采购条件。
...
- **停止功能与制动设计（决定保护场尺寸，返工代价最高）**：STO 只是切除力矩，**不等于停下来**——底盘切矩后会滑行，臂的重力轴会下坠。必须：① 底盘按 SS1-t 最坏路径（受控减速失败 → 定时 STO → 抱闸吸收动能）实测停止距离，以此定保护场尺寸，抱闸按动态制动能量和紧急制动次数寿命校核；② 臂重力轴必须配机械抱闸且掉电即抱；③ **制动距离实测提前到样机阶段**，不要拖到认证。
...
- **移动中臂锁定收拢姿态**（可加位置开关做硬件监测，作为未来认证升级位）；
...
### 5.1 总线选型：臂和底盘统一 EtherCAT

- 臂：6 轴 1kHz 同步插补，CANopen 1Mbps 带宽和 SYNC 抖动都不够，**必须 EtherCAT**（DC 分布式时钟轴间同步 <1µs）。
- 底盘：CANopen 技术上够，但两条总线 = 两套主站/调试工具/故障模式。**结论：同挂一条 EtherCAT 链**，维护成本减半。代价是单链单点故障（§3.4 已述），第一版接受。
- **伺服采购硬性条件清单**（选型阶段就要拍板，事后返工代价极大）：
  EtherCAT + CiA 402 profile + 带证书的 STO 端子 + **多圈绝对值编码器**（否则每次上电 6 轴都要回零，自研库还得实现 Homing 模式）+ **驱动器内建抱闸管理（时序参数可配）** + 支持 CST 力矩模式（力控远期留口）。

### 5.2 主站与自研驱动库

- 主站选 **IgH EtherCAT Master**（内核态；SOEM 用户态易上手但 DC 同步和实时性要自己啃，仅原型期备选）。**产品化落地风险要正视**：IgH 是 out-of-tree 内核模块，对新内核和 native 网卡驱动的支持常滞后——锁定内核版本，把 IgH 模块纳入 DKMS/镜像构建流水线，**OTA 时内核与 IgH 绑定为同一原子更新单元**；选型阶段先在目标内核上实测 `ec_igb` 补丁可用性。
- **自研一个"与 ROS 无关"的 C++ 驱动库**（`ecat402_lib`）：封装 CiA 402 状态机（含 Fault Reset 路径）、CSP/CSV 模式、SDO 参数下发、PDO 映射，**附命令行示波/调试工具**——售后可脱离 ROS 整栈单独排障，这个库是核心资产，ROS 版本迁移不动它。
...
把自研驱动库封装为 `hardware_interface::SystemInterface` 插件（可参考 ICube-Robotics/`ethercat_driver_ros2` 的做法，**但只作参考实现，不引入依赖**——引入它就丢掉了"EtherCAT 层零 ROS 依赖"的核心卖点）。controller_manager 以 1kHz 跑在隔离核（systemd 原生服务，不进容器），加载：

- `joint_trajectory_controller`（臂，位置接口，接 MoveIt）；
- `diff_drive_controller`（底盘，速度接口，接 Nav2；**cmd_vel 超时直接用它自带的 `cmd_vel_timeout` 参数**，不用自研）；
- `joint_state_broadcaster`（**必配**——不发 `/joint_states`，MoveIt 和 TF 直接不能工作，新手常漏）；
- 夹爪控制器（若夹爪挂 EtherCAT：Jazzy 的 `parallel_gripper_action_controller`；若是独立串口/IO 设备则由厂商驱动节点提供同名 action，L6 接口不变）；
- 自研安全监控 chainable controller——职责收敛为：**驱动器故障聚合 + Quick Stop 触发 + 一致性校验 +"臂未收拢则钳零 cmd_vel"互锁强制**。软限位/超速用 ros2_control 自带的 joint limits enforcement，别重复造轮子。

臂和底盘共用一个 controller_manager / 一条总线 / 一个 1kHz 环（底盘控制器内部分频即可），不要另起第二个实时环。
...
- **本层崩溃**：EtherCAT 从站 SM 看门狗必须使能，超时驱动器进入配置好的故障反应（CiA 402 对象 605Eh，逐轴确认配置）+ 抱闸管理时序确认——"自主抱闸"不是默认白送的行为。
...
- **URDF/xacro 是单一事实来源**：底盘 + 臂 + 末端 + 全部传感器外参挂一棵树，Nav2 costmap、MoveIt 碰撞、TF、手眼外参全部从这里出。臂段连杆参数用 CAD 导出并实测校核；惯性参数认真填（未来力控要用）。
- 传感器驱动全用官方包：`sick_scan_xd` / `rslidar_sdk`（按雷达选型）、`realsense2_camera` / `OrbbecSDK_ROS2`（相机，见 §7.2 选型）、IMU 驱动。
- 里程计融合：`robot_localization` EKF（轮 odom + IMU → `odom→base_link`，开 `two_d_mode`）。**新手第一大坑：`diff_drive_controller` 的 `enable_odom_tf` 必须设 false**，否则与 EKF 争发同一段 TF，定位莫名漂移抖动。
...
- **布局**：腕部 eye-in-hand 深度相机管抓取（随臂逼近、最后一拍在 30~50cm 处，精度自然收敛）；固定前视 1~2 颗管避障（避障视野不能依赖臂姿态）。
...
- footprint 用收拢姿态实际外轮廓多边形（含夹爪/载箱最大突出量），不用内切圆；空载/持箱两套参数切换。
...
规划（move_group，非实时容器）与执行（JTC，1kHz 实时环）分离，之间的下行运动通道只有 FollowJointTrajectory 一条；上行依赖 `/joint_states` 等状态回流（接口清单里要列全）。**诚实的失效语义**（见 §5.6）：轨迹一旦被 JTC 接受，上层崩溃不会自动停臂——执行期监护（谁存活、谁负责 cancel goal）必须显式设计，安全事件（急停恢复、场入侵）与 JTC 的交互（cancel/hold/恢复流程）要有明确定义。

### 9.2 模型与规划器

- Setup Assistant 生成 SRDF：`arm`/`gripper` 组、`tcp` 系、命名位姿（home/**tuck**/transport）、自碰撞矩阵。碰撞体一律简化几何（盒/圆柱/凸包），可视化 mesh 与碰撞 mesh 分开。
...
2. **L1 强制执行（关键）**：实时环里的安全监控控制器读关节实际位置，臂不在收拢包络内就钳零 cmd_vel；MANIPULATING 态钳零底盘并保持抱闸。**绝不能只靠行为树**——树崩溃/暂停时靠下层兜底；
3. **A 域预留**：未来认证时加臂位置硬件开关进安全 PLC。

互锁信号定义为两个带确认的状态量：`arm_stowed`、`base_parked`，作为行为树中移动/作业动作的硬前置条件。
...
- 每个主干阶段外包恢复装饰：导航失败 `Retry(3)` → 备选停靠点 → 降级；视觉找不到目标 → 换观察视点重拍 → 微调停靠 → 上报待远程处置；抓取失败（电流判空抓）→ 退回观察位重估重抓 `Retry(2)` → 标记失败转人工。所有耗尽路径汇入同一**降级子树**：臂收拢 → 底盘停稳 → DEGRADED → 触发黑匣子快照 → 上报。
...
4. **伺服采购漏条件**——EtherCAT+CiA402+带证书 STO+多圈绝对值编码器+抱闸管理+CST，少一条日后返工。
...
| 3 | 臂+底盘同挂一条 EtherCAT | 一套主站/工具/故障模式 | 单链单点故障→整机停（可接受，量产前评估分链） |
